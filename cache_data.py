...
df = apply_fracdiff(df, cutoff=cutoff)
clean(df)

indf = indf[cutoff:]

assert len(indf) == len(df)
...
test_df = df.iloc[ldf-cutoff:]

print(len(train_df))
print(len(test_df))

table = pa.Table.from_pandas(train_df)
pq.write_table(table, './data/FRACDIFF_DATA.parquet')

table = pa.Table.from_pandas(test_df)
pq.write_table(table, './data/FRACDIFF_TEST.parquet')
