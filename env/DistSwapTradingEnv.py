...
class DistSwapTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.commission = kwargs.get('commission', 0.000)
    ...
    def _take_action(self, action):
        ...
        if action != self.prev_action:
            ...
            short_dist = margin_available * short_action
            long_dist = margin_available * long_action

            next_short = int((short_dist*self.leverage) / price_per_contract)
            next_long = int((long_dist*self.leverage) / price_per_contract)
            next_balance = equity - ((next_short/self.leverage) * price_per_contract + (next_long/self.leverage) * price_per_contract)
